...
сокет-файлу; при это сервис будет называться _php56-php-fpm_.
* Можно донастроить дополнительные php-fpm к версии php, со своими переменными окружения, настройками php или другими опциями и конечно со своим сокет-файлом.
* Nginx пробрасывает запросы к php файлам на сокет-файл php-fpm.
* В конфигурации php-fpm: `user = nginx`, `group = nginx`, `listen.acl_users = apache,nginx`.
